...
export default {
  ...
  methods: {
    loadData() {
      this.$http.get("/api/spec/params?gid=" + this.group.id).then(({ data }) => {
          data.forEach(p => {
    ...
    deleteParam(id) {
        this.$message.confirm("确认要删除该参数吗？").then(() => {
            this.$http.post('/api/spec/param/' + id).then(() => {
    ...
    save(){
      if(this.isEdit){
        console.log("edit,edit,edit,edit,edit,edit");
        // console.log("param",param)
        this.param.segments = this.param.segments.map(s => s.join("-")).join(",");
        this.$http({
          contentType: "application/json;charset=UTF-8",
          dataType: 'json',
          method: 'post',
          url: '/api/spec/param/update',
          data:this.param
        }).then(data => {
